...
public:
    ...
    // Ownership of iceCreamData must already have been transferred
    // (or will be cleaned by the destructor if still waiting).
    void dequeue() {
        if (isEmpty()) return;
...
int main() {
    ...
    // Destructors of activeQueue and saleHistory will run automatically
    // and free every remaining IceCream object safely.
    return 0;
}
